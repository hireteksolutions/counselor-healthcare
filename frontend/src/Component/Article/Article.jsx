...
function Article() {
    ...
    return (
        <div className='pt-8 pb-8 p-1 h-[100%]'>
            <h1 className='mt-4 mb-5 text-center text-4xl font-semibold'>
                    ...
                    {slides.map((slide, index) => (
                        <div key={index} className={style.slide}>
                            <div className={style.slideContent}>
                                ...
                                <Link to={slide.buttonLink} className={style.button}>{slide.buttonText}<IoMdArrowRoundForward /></Link>
                            </div>
                        </div>
                    ))}
                </Slider>

                <button className={style.prevButton} onClick={() => sliderRef.current.slickPrev()}><IoArrowBackOutline /></button>
                <button className={style.nextButton} onClick={() => sliderRef.current.slickNext()}><IoMdArrowRoundForward /></button>
            </div>
